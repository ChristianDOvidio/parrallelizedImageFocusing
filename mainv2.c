...
ImageStruct *imageReadArray;			  //array of images to read
pthread_mutex_t myDataLock;				  //mutex to lock data
...
void initializeApplication(void)
{
	...
	pthread_mutex_init(&myDataLock, NULL); //initialize mutex
		...
		for (int windowX = pixelLX; windowX < pixelRX; windowX++)
		{
			for (int windowY = pixelBY; windowY < pixelTY; windowY++)
			{
				...
				{ //if pixel value at that location in the output image is 0
					...
					pthread_mutex_lock(&myDataLock); //enter CS
					...
					pthread_mutex_unlock(&myDataLock); //leave CS
				}
				else
