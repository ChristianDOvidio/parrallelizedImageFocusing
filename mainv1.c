...
#define windowSize 7 //window should be 5 or 7

ImageStruct *imageReadArray;
...
void *imageCorrect(void *argument)
{
	...
	for (int pixelX = info->rowStart; pixelX < info->rowEnd; pixelX++)
	{ //for each pixel in the rectangular window
		for (int pixelY = info->colStart; pixelY < info->colEnd; pixelY++)
		{
			//printf("Thread %d working on pixel %d,%d\n", info->index, pixelX, pixelY);
			...
			{ //look at all input files
				...
				//jyh: initialize minGreyValue and maxGreyValue
				...
				{ //find which image is most focused at window
					maxFocus = focusScore;
					//printf("Thread %d Max focus: %d\n", info->index, maxFocus);
					maxFocusImage = imageStackCount;
					//printf("Thread %d Max focus image: %d\n", info->index, maxFocusImage);
				}
			}
			...
			unsigned char redValue = bestImage[pixelX * imageReadArray[0].bytesPerRow + 4 * pixelY + 0];
			unsigned char greenValue = bestImage[pixelX * imageReadArray[0].bytesPerRow + 4 * pixelY + 1];
			unsigned char blueValue = bestImage[pixelX * imageReadArray[0].bytesPerRow + 4 * pixelY + 2];

			info->raster[pixelX * info->outputImage->bytesPerRow + 4 * pixelY + 0] = redValue;
			info->raster[pixelX * info->outputImage->bytesPerRow + 4 * pixelY + 1] = greenValue;
			info->raster[pixelX * info->outputImage->bytesPerRow + 4 * pixelY + 2] = blueValue;
			//printf("Replacing pixel %d,%d\n", pixelX, pixelY);
		}
	}
